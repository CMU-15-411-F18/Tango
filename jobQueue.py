...
class JobQueue:
    ...
    def findRemovingWaiting(self, outputFile):
        ...
        with self.queueLock:
            ...
            liveJobResult = self.liveJobs.wrapped.get(outputFile)
            deadJobResult = self.deadJobs.wrapped.get(outputFile)

            if liveJobResult:
                (id, job) = liveJobResult
                status = JobQueue.JobStatus.RUNNING if j.assigned else JobQueue.JobStatus.WAITING
            elif deadJobResult:
                (id, job) = liveJobResult
